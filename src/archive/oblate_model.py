...
import model_transits as transit
...
def partial_occultation(flags, alphas, x_intersect, y_intersect, x0, y0, rp_eq, f, u1, u2):
...
def compute_oblate_transit_lightcurve(transit_parameters, time_array, exp_time=0.02, supersample_factor=5):
    ...
    ##
    INTERSECT = flags==0
    delta_flux[INTERSECT] = partial_occultation(flags[INTERSECT], alphas[INTERSECT], x_intersect[INTERSECT], y_intersect[INTERSECT], x0[INTERSECT], y0[INTERSECT], rp_eq, f, u_1, u_2)
    FULL = flags==1
...
def test_kepler167():
    ...
    transit_parameters = np.array([t_0, b_0, period, rp_eq, f, obliquity, u_1, u_2, log10_rho_star])
    time_array = np.linspace(t_0-12/24., t_0+12/24., 1000)
